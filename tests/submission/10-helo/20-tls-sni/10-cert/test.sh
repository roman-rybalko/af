...
swaks -tls --tls-cert client.crt --tls-key client.key --tls-sni submission2.submission-test.clients.advancedfiltering.net -f test@test.advancedfiltering.net -t mbox@test.com -s $DST_HOST -p submission -q helo | grep O=advancedfiltering | grep OU=tests | grep CN=submission
